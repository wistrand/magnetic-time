...
pub fn to_json(face: &FaceConfigs, sim: &SimParams, style: &Style, speed: f64) -> String {
    ...
    let face_color = style.face_color.map(&hex).unwrap_or_default();
    e.push(format!("{}: {}", q("face_color"), q(&face_color)));
    ...
    num_line(&mut e, "fps_cap", style.fps_cap as f64);
    ...
    e.push(format!("{}: {}", q("flip_x"), style.flip_x));
    e.push(format!("{}: {}", q("flip_y"), style.flip_y));
...
pub fn apply_json(
    ...
    if let Some(v) = text("face_color") {
        style.face_color = if v.is_empty() {
            None
        } else {
            parse_color(v).ok()
        };
    ...
    if let Some(v) = num("fps_cap") {
        style.fps_cap = v.clamp(0.0, 1000.0) as u32;
    }
    ...
    if let Some(v) = flag("flip_x") {
        style.flip_x = v;
    }
    if let Some(v) = flag("flip_y") {
        style.flip_y = v;
    }
